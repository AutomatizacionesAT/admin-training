...
import { CoordinatorDetailTable } from "./CoordinatorDetailTable";
...
function StatusCard({ label, value, colorClass, bgClass, dotClass, iconPath, rotateClass = "" }: StatusCardProps) {
  return (
    <div className={`relative overflow-hidden bg-white rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border ${colorClass} p-8 flex items-center justify-between hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group`}>
      <div className={`absolute -left-6 -bottom-6 w-24 h-24 ${bgClass} rounded-full blur-2xl group-hover:scale-150 transition-transform duration-700`} />
      <div className="relative z-10">
        <div className="flex items-center gap-2 mb-2">
          <div className={`w-2 h-2 rounded-full ${dotClass} ${label === "Finalizados" || label === "En proceso" ? "animate-pulse" : ""}`} />
          <h4 className="text-sm font-semibold text-gray-500 uppercase tracking-wider">{label}</h4>
        </div>
        <span className="text-5xl font-black text-gray-800 tracking-tight">{value}</span>
      </div>
      <div className={`relative z-10 w-16 h-16 ${bgClass} rounded-2xl flex items-center justify-center shadow-inner ${rotateClass} transition-transform duration-300`}>
...
export function ReportTab({ 
  ...
  const statusValues = [reportData.finalizados, reportData.enProceso, reportData.proyectados];
    ...
    <div className="min-h-screen">
      <div className="mb-6 flex flex-wrap items-center gap-4 bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
        <div className="flex items-center gap-3">
          <label className="text-sm font-semibold text-gray-600 uppercase tracking-wider">Año</label>
          <select 
            value={selectedYear} 
            onChange={(e) => setSelectedYear(Number(e.target.value))}
            className="bg-gray-50 border border-gray-200 text-gray-800 text-sm rounded-xl focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5 outline-none hover:border-indigo-300 transition-colors cursor-pointer"
          >
        ...
        <div className="flex items-center gap-3">
          <label className="text-sm font-semibold text-gray-600 uppercase tracking-wider">Mes</label>
          <select 
        ...
        <div className="flex items-center gap-3">
          <label className="text-sm font-semibold text-gray-600 uppercase tracking-wider">Dirección</label>
          <select 
            value={selectedDireccion === null ? "" : selectedDireccion} 
            onChange={(e) => setSelectedDireccion(e.target.value === "" ? null : e.target.value)}
            className="bg-gray-50 border border-gray-200 text-gray-800 text-sm rounded-xl focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5 outline-none hover:border-indigo-300 transition-colors cursor-pointer"
          >
        ...
        <div className="col-span-1 lg:col-span-5 h-full">
          ...
          >
            {reportData.industrias.map((ind, idx) => (
              <div key={ind.nombre} className="bg-white rounded-[1.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100/60 p-5 flex flex-col justify-between hover:-translate-y-1 hover:shadow-lg transition-all duration-300">
                <div className="flex justify-between items-start mb-3">
                  <div className={`p-2 rounded-xl ${DIR_BG_COLORS[idx % DIR_BG_COLORS.length]}`}>
                ...
                <div>
                  <h4 className="text-sm font-semibold text-gray-600 mb-1 line-clamp-2 leading-tight" title={ind.nombre}>
                    {ind.nombre}
                  </h4>
                  <div className="flex items-end justify-between mt-2">
                    <span className="text-3xl font-black text-gray-800 tracking-tight">{ind.count}</span>
                  </div>
                  <div className="w-full bg-gray-100 rounded-full h-1.5 mt-3 overflow-hidden">
                    <div className={`h-full rounded-full bg-linear-to-r ${DIR_COLORS[idx % DIR_COLORS.length]}`} style={{ width: `${ind.porcentaje}%` }} />
                  </div>
                </div>
              </div>
            ))}
          ...
          <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-2">
            {reportData.coordinadores.map((coord, idx) => {
              ...
              return (
                <button
                  key={coord.nombre}
                  onClick={() => onSelectCoordinador(isSelected ? null : coord.nombre)}
      ...
      {selectedCoordinador && (
        <CoordinatorDetailTable data={data} selectedCoordinador={selectedCoordinador} />
      )}
    </div>
  );
}
